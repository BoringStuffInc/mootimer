...
async fn test_rpc_crud_operations() -> Result<()> {
    ...
    let p_res = client.profile_create("test-profile", "Test Profile", None).await?;
    ...
    let t_res = client.task_create("test-profile", "Delete Me Task", None).await?;
    let task_id = t_res["id"].as_str().expect("Task ID should be string").to_string();
